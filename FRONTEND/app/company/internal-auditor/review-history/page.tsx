...
const REVIEW_HISTORY_KEY = "finshield_review_history"
...
const SEED_LOGS: ReviewHistoryEntry[] = [
    {
    ...
    },
    {
        ...
        reason: "Requires owner review due to large amount and unresolved vendor query."
    },
]

function getLogs(): ReviewHistoryEntry[] {
    if (typeof window === "undefined") return SEED_LOGS
    try {
        const stored = localStorage.getItem(REVIEW_HISTORY_KEY)
        if (stored) {
            const parsed: ReviewHistoryEntry[] = JSON.parse(stored)
...
export default function ReviewHistoryPage() {
    const [logs, setLogs] = useState<ReviewHistoryEntry[]>([])
    ...
    const load = () => {
        ...
        setLogs(l)
        localStorage.setItem(REVIEW_HISTORY_KEY, JSON.stringify(l))
    }
    ...
    function downloadCSV() {
        ...
        a.download = `review-history-${Date.now()}.csv`
        a.click()
    }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
                <div>
                    <h2 className="text-2xl font-normal tracking-tight">Review History</h2>
