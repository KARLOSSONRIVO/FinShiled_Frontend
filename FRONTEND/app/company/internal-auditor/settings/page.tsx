                    ...
                    <TabsList className="relative bg-muted p-1 rounded-lg inline-flex overflow-x-auto no-scrollbar">
                        <div className="absolute inset-0 p-1 pointer-events-none">
                            <motion.div
                                ...
                                layoutId="auditorSettingsTabIndicator"
                                initial={false}
                                animate={{ x: `${tabIndex * 100}%`, width: `${100 / tabCount}%` }}
                        ...
                        <TabsTrigger value="appearance" className="gap-2 w-36 relative z-10 px-6 py-1.5 data-[state=active]:bg-transparent data-[state=active]:shadow-none focus-visible:ring-0">
                            <Shield className="h-4 w-4" /> Appearance
                        </TabsTrigger>
                    </TabsList>
                </div>
                        ...
                        <div className="p-4 sm:p-6 border rounded-xl bg-card text-card-foreground shadow-sm space-y-6 w-full overflow-hidden">
                            ...
                            <div className="grid gap-4 w-full sm:max-w-xl">
                                <div className="grid gap-2">
                                    ...
                                    {user?.username ? (
                                        <Input value={user.username} readOnly className="bg-muted cursor-default focus-visible:ring-0 select-none" />
                                    ) : <Skeleton className="h-10 w-full" />}
                                </div>
                                <div className="grid gap-2">
                                    <Label>Email</Label>
                                    {user?.email ? (
                                        <Input value={user.email} readOnly className="bg-muted cursor-default focus-visible:ring-0 select-none" />
                                    ) : <Skeleton className="h-10 w-full" />}
                                </div>
                                <div className="grid gap-2">
                                    <Label>Role</Label>
                                    {user?.role ? (
                                        <Input value={formatRole(user.role)} readOnly className="bg-muted cursor-default focus-visible:ring-0 select-none" />
                                    ) : <Skeleton className="h-10 w-full" />}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-4 sm:p-6 border rounded-xl bg-card text-card-foreground shadow-sm space-y-6 w-full overflow-hidden">
                            ...
                            <div className="flex items-center justify-between bg-muted/50 p-4 rounded-lg">
                                <div>
                                    ...
                                    <p className="text-xs text-muted-foreground">Update your password to a new one.</p>
                                </div>
                                <Button
