...
import { groupInvoicesByKey } from "@/lib/chart-utils"

export function useSuperAdminReports() {
    const { data: allInvoices = [], isLoading } = useQuery({
        queryKey: ["invoices", "super-admin-reports"],
        queryFn: async () => {
        ...
        .filter(
            (i) =>
                ...
                (String(i.reviewDecision) !== "rejected" &&
                    (i.aiVerdict?.verdict === "clean" ||
                        (!i.aiVerdict?.verdict &&
                            String(i.status) !== "flagged" &&
    ...
    const statusCounts = {
        approved: allInvoices.filter(
            (i) =>
                String(i.status) === "approved" ||
                String(i.reviewDecision) === "approved"
        ).length,
        rejected: allInvoices.filter(
            (i) =>
                String(i.status) === "rejected" ||
                String(i.reviewDecision) === "rejected"
        ).length,
    ...
    const averageRiskScore =
        allInvoices.length > 0
            ? allInvoices.reduce(
                  (sum, inv) => sum + (Number(inv.aiVerdict?.riskScore) || 0),
                  0
              ) / allInvoices.length
            : 0

    const verifiedCount = allInvoices.filter(
        (i) => i.aiVerdict?.verdict === "clean"
    ).length
    const aiFlaggedCount = allInvoices.filter(
        (i) => i.aiVerdict?.verdict === "flagged"
    ).length

    const fraudCount = allInvoices.filter(
        (i) =>
            (String(i.status) === "flagged" ||
                i.aiVerdict?.verdict === "flagged") &&
            String(i.reviewDecision) !== "approved" &&
    ...
    const companyBreakdown = groupInvoicesByKey(allInvoices, "companyName")
