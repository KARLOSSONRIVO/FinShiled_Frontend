    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Technical Polygon Amoy transaction and anchoring status.</p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-6 text-red-800">{error || "Blockchain Transactions could not be loaded."}</div>
      ) : (
        <BlockchainTable
          invoices={invoices}
