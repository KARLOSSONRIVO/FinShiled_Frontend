...
const labels: Record<string, string> = { api: "API", database: "Database", redis: "Redis", ai: "AI Service", blockchain: "Blockchain Network", email: "Email Service" }

export function SystemStatusPanel({ compact = false }: { compact?: boolean }) {
  const { data, isLoading, isError, refetch } = useSystemStatus()
  if (isLoading) return <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="Loading system status">{Array.from({ length: 6 }).map((_, index) => <Skeleton key={index} className="h-32 rounded-2xl" />)}</div>
  ...
  return <div className="space-y-5"><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{data.services.filter((service) => service.name !== "ipfs").map((service) => {
